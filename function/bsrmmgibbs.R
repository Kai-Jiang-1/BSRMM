bsrmmgibbs <- function(nburnin, niter, p, nop, Y, X, N, a, Q, n, tau, nu, omega,
  ...
  alpha[1] <- rbeta(1, shape1 = alpha_a_shape, shape2 = alpha_b_shape)

  # for both sigma2 and alpha, we have one more nubmer because we need to sampling them firstly. 
  # we will replace them after 1st iteration. 
    ...
    y_mvi[, 1] <- sapply(1:n, function(s) {
      if (missing_index[s, 1] == 1) {
        return(
          ...
          # * stand$Sy[[1]] + stand$muy[[1]] # smaller than lod
        )
        ...
        return(
          ...
          # * stand$Sy[[1]] + stand$muy[[1]] # greater than lod
        )
      } else {
        return(Y[s, 1])
      }
  ...
  for(i in 1:(nburnin+niter)){
    # print tembeta by using cat 
    # print(i)
    # cat("The sum of tembeta is ", sum(tembeta), "\n")
    # BAZE for variable selection 
    proposeindx <- sample(1:p, 1, replace = TRUE) # purpose features for initialize 
    gamma[i+1,] <- gamma[i,]
    
    flag <- any(index == proposeindx) & (length(index) > 1)
    # print(flag)
    # print(any(index != proposeindx))
    if (flag) {
    ...
    if (bayestmetab == TRUE) {
      ...
      # Ri = 0 = missing
      # Ri = 1 = observed
      alpha_a <- sum(missing_index[, i + 1] == 0) + alpha_a_shape
      alpha_b <- sum(missing_index[, i + 1] == 2) + alpha_b_shape
