...
if (correlated == "cor") {
  ...
  result_path <- paste0("./result/diag_bsrmm_", imputation_method, "_imputation_", missing_type, "_cor_", n, "_", p, "_", snr, "_", proportion, "_", jobid, ".RDS")
} else {
  mydata_path <- paste0("./data/", imputation_method, "_", missing_type, "_ind_", n, "_", p, "_", snr, "_", proportion, "_", jobid, ".RDS")
  result_path <- paste0("./result/diag_bsrmm_", imputation_method, "_imputation_", missing_type, "_ind_", n, "_", p, "_", snr, "_", proportion, "_", jobid, ".RDS")
}
...
for (i in 1:num_chains) {
  ...
  all_chains_bsrmm_output[[i]] <- bsrmmgibbs(nburnin, niter, p, nop, Y, X, N, a, Q, n, tau, nu, omega, 
                                             alpha_a_shape = 1, alpha_b_shape = 1,
                                             seed, predict_result, bayestmetab, display, lod_ratio = 1)
...
for (chain_idx in 1:num_chains) {
  ...
  # scale back the tembeta to original dimension 
  # beta_samples <- apply(tembeta_samples, 2, function(x) x / stand$Sx[which(PPI > 0.5)])
  
  # scale back the beta_0 
  # beta_0_samples <- beta_0_samples - stand$Sx[which(PPI > 0.5)] %*% beta_samples
...
# Savae the Rhat as HTML 
rhat %>%
  kable("html", caption = "") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE) %>%
  save_kable("./result/rhat_bsrmm.html")

pdf("./plot/traceplot_bsrmm.pdf", width = 12, height = 8)
par(mfrow = c(2, 3)) 
